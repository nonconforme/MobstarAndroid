...
public class CropImageView extends ImageView {
    ...
    private static final int TOUCH_BUFFER = 20;
    ...
    private static boolean isActionInsideRectangle(float x, float y) {
        return (x>=(leftTop.x- TOUCH_BUFFER)&&x<=(rightBottom.x+ TOUCH_BUFFER)&& y>=(leftTop.y- TOUCH_BUFFER)&&y<=(rightBottom.y+ TOUCH_BUFFER))?true:false;
    }
    ...
    private static int getAffectedSide(float x, float y) {
        if(x>=(leftTop.x- TOUCH_BUFFER)&&x<=(leftTop.x+ TOUCH_BUFFER))
            return LEFT;
        else if(y>=(leftTop.y- TOUCH_BUFFER)&&y<=(leftTop.y+ TOUCH_BUFFER))
            return TOP;
        else if(x>=(rightBottom.x- TOUCH_BUFFER)&&x<=(rightBottom.x+ TOUCH_BUFFER))
            return RIGHT;
        else if(y>=(rightBottom.y- TOUCH_BUFFER)&&y<=(rightBottom.y+ TOUCH_BUFFER))
            return BOTTOM;
        else
            return DRAG;
    }
